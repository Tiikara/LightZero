...
class RepresentationNetworkUniZeroCapsSEMOnly(nn.Module):
    ...
    def __init__(
        ...
        self.downsample_net_out_features = num_channels * (self.observation_shape[1] // 8) * (self.observation_shape[2] // 8)

        self.out_capsules = (num_capsules, out_capsules_dim) # num_capsules x (embedding_dim / num_capsules) = embedding_dim
        self.cap_sem = nn.Sequential(
            nn.Linear(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.cap_sem(x.reshape(-1, self.downsample_net_out_features))
