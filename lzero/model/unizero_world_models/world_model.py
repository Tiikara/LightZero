...
class WorldModel(nn.Module):
    ...
    def compute_loss(self, batch, target_tokenizer: Tokenizer = None, inverse_scalar_transform_handle=None, **kwargs: Any) -> LossWithIntermediateLosses:
        ...
        if self.obs_type == 'image':
            ...
            if self.latent_recon_loss_weight != 0. or self.perceptual_loss_weight != 0.:
        ...
        elif self.predict_latent_loss_type == 'barlow_twins_proj_real_pred_log_cosh':
            logits_observations_proj = self.tokenizer.encoder.projection_model(logits_observations)
            with torch.no_grad():
                labels_observations_proj = target_tokenizer.encoder.projection_model(labels_observations)
        ...
        if self.reg_type == 'vic':
            ...
            loss_vic = VICRegSingleLoss(var_coeff = 1.0, cov_coeff = 0.04)(
                logits_observations_proj
            ) + 0.1 * F.mse_loss(logits_observations_proj, labels_observations_proj)
